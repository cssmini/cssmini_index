(function() {
    ...
    document.querySelectorAll('[data-lang-option]').forEach(function(a) {
        a.classList.toggle('active', a.dataset.langOption === lang);
    });

    // paginator: translate prev/next links generated by Hexo
    var prev = document.querySelector('#page-nav .extend.prev');
    var next = document.querySelector('#page-nav .extend.next');
    if (prev && t['paginator.prev']) prev.textContent = t['paginator.prev'];
    if (next && t['paginator.next']) next.textContent = t['paginator.next'];
